...
func GetDNSConfig(ctx context.Context, k8sClient client.Client) (cappv1alpha1.DNSConfig, error) {
	cappConfig := v1alpha1.CappConfig{}
	if err := k8sClient.Get(ctx, types.NamespacedName{Namespace: CappNS, Name: mocks.CappConfigName}, &cappConfig); err != nil {
		return cappv1alpha1.DNSConfig{}, fmt.Errorf("could not fetch cappConfig %q from namespace %q: %w", mocks.CappConfigName, CappNS, err)
	}
