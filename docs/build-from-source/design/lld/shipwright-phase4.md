...
- `clusterBuildStrategy.buildFile.present` (string; optional):
  - Name of the `ClusterBuildStrategy` to use when the source indicates a
    file-based build (e.g. a `Dockerfile`) (platform default).
- `clusterBuildStrategy.buildFile.absent` (string; optional):
  - Name of the `ClusterBuildStrategy` to use when the source does not indicate
    a file-based build (platform default).
